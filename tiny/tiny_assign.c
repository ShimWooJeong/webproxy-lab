...
void doit(int fd)
{
    ...
    */
    ...
    if (is_static)
    {
        ...
        {
            clienterror(fd, filename, "403", "Forbidden", "Tiny couldn't read this file", version);
            return;
        }
        // 위 조건을 다 만족한다면 -> 정적 컨텐츠를 클라이언트에게 제공
        serve_static(fd, filename, sbuf.st_size, method, version); // method 인자 추가=과제?
    }
    else
    {
        ...
        serve_dynamic(fd, filename, cgiargs, method, version); // method 인자 추가=과제?
    }
}
...
void serve_static(int fd, char *filename, int filesize, char *method, char *version)
{
    ...
    // 원래는 파일 열어서 클라에게 보내주는데, HEAD라면 그냥 리스폰스 헤더만 보고 싶으니까 안 띄우고 그냥 정보만 본다@@@@@!!!!
    //  11.9) mmap/rio_writen 대신 malloc/rio_readn/rio_writen으로 연결 식별자에게 복사하도록
    srcp = malloc(filesize);          // mmap은 malloc과 유사하지만, 값 복사까지 해줌
    Rio_readn(srcfd, srcp, filesize); // malloc은 메모리 공간 할당만 해주기 때문에 Rio_readn으로 값을 넣어줘야 함
    Close(srcfd);
